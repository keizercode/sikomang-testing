...
.leaflet-popup-content::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 3px; }
.leaflet-popup-content::-webkit-scrollbar-thumb { background: #888; border-radius: 3px; }
.leaflet-popup-content::-webkit-scrollbar-thumb:hover { background: #555; }
...
/* Collapsed state: tampil sebagai icon bulat kecil */
...
/* Toggle button (ikon layers bawaan Leaflet) */
...
<script>
    ...
    // ===========================
    // LAYER CONTROL (basemaps + overlay density)
    // ===========================
    ...
    // Label HTML dengan swatch warna untuk setiap density
    const overlayLabel = (color, stroke, text) =>
        `<span style="display:inline-flex;align-items:center;gap:6px;">
            <span style="display:inline-block;width:14px;height:14px;border-radius:3px;
                         background:${color};border:2px solid ${stroke};flex-shrink:0;"></span>
            <span>${text}</span>
         </span>`;

    const overlayMaps = {
        [overlayLabel('#8dd3c7', '#5aa89a', 'Mangrove Jarang')]: layers.jarang,
        [overlayLabel('#FFFFB3', '#d4d466', 'Mangrove Sedang')]: layers.sedang,
        [overlayLabel('#BEBADA', '#9186ad', 'Mangrove Lebat')]:  layers.lebat,
    };
    ...
    function createPopupContent(feature) {
        ...
        try {
            ...
            if (kawasan) html += `<span style="background:#009966;color:white;">${kawasan}</span>`;
            if (props.KODE_PROV) html += `<span style="background:#1f2937;color:white;font-size:10px;">Prov: ${props.KODE_PROV}</span>`;
            html += '</div>';
            ...
            if (hasWilayah) {
                html += `<div style="margin-top:12px;padding:10px;background:#f9fafb;border-radius:6px;">
                    <div style="font-size:11px;font-weight:700;color:#1f2937;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">📍 Informasi Wilayah</div>`;
                if (props.WADMKK) html += `<div class="popup-info"><strong>Kabupaten/Kota:</strong> ${props.WADMKK}</div>`;
                if (props.WADMPR) html += `<div class="popup-info"><strong>Provinsi:</strong> ${props.WADMPR}</div>`;
                if (props.KODE_PROV) html += `<div class="popup-info"><strong>Kode Provinsi:</strong> ${props.KODE_PROV}</div>`;
                html += '</div>';
            }

            const hasMangrove = props.KTTJ || props.STRUKTUR_V || props.LSMGR || props.Shape_Leng || props.Shape_Area;
            if (hasMangrove) {
                html += `<div style="margin-top:10px;padding:10px;background:#ecfdf5;border-radius:6px;">
                    <div style="font-size:11px;font-weight:700;color:#065f46;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">🌳 Data Mangrove</div>`;
                if (props.KTTJ) html += `<div class="popup-info"><strong>Kategori:</strong> ${props.KTTJ}</div>`;
                if (props.STRUKTUR_V) html += `<div class="popup-info"><strong>Struktur Vegetasi:</strong> ${props.STRUKTUR_V}</div>`;
                if (props.LSMGR) {
                    const lsmgr = parseFloat(props.LSMGR);
                    const luasDisplay = lsmgr > 1000
                        ? `${(lsmgr / 10000).toFixed(4)} ha (${lsmgr.toFixed(2)} m²)`
                        : `${lsmgr.toFixed(4)} ha`;
                    html += `<div class="popup-info"><strong>Luas Mangrove:</strong> ${luasDisplay}</div>`;
                }
                if (props.Shape_Leng) html += `<div class="popup-info"><strong>Shape Length:</strong> ${parseFloat(props.Shape_Leng).toFixed(8)}</div>`;
                if (props.Shape_Area) html += `<div class="popup-info"><strong>Shape Area:</strong> ${parseFloat(props.Shape_Area).toFixed(8)}</div>`;
                html += '</div>';
            }

            const hasKawasan = kawasan || props.FUNGSIKWS || props.KONSERVASI || props.LSKKWS;
            if (hasKawasan) {
                html += `<div style="margin-top:10px;padding:10px;background:#fef3c7;border-radius:6px;">
                    <div style="font-size:11px;font-weight:700;color:#92400e;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">🏛️ Informasi Kawasan</div>`;
                if (kawasan) html += `<div class="popup-info"><strong>Jenis Kawasan:</strong> ${kawasan}</div>`;
                if (props.FUNGSIKWS) html += `<div class="popup-info"><strong>Fungsi Kawasan:</strong> ${props.FUNGSIKWS}</div>`;
                if (props.KONSERVASI) {
                    const cc = props.KONSERVASI.toLowerCase().includes('kawasan konservasi') ? '#16a34a' : '#f59e0b';
                    html += `<div class="popup-info" style="color:${cc};font-weight:600;"><strong>Status Konservasi:</strong> ${props.KONSERVASI}</div>`;
                }
                if (props.LSKKWS) html += `<div class="popup-info"><strong>Luas SK Kawasan:</strong> ${parseFloat(props.LSKKWS).toLocaleString('id-ID')} ha</div>`;
                html += '</div>';
            }

            const hasLegalitas = props.NOSKKWS || props.TGLSKKWS || props.INTS;
            if (hasLegalitas) {
                html += `<div style="margin-top:10px;padding:10px;background:#e0f2fe;border-radius:6px;">
                    <div style="font-size:11px;font-weight:700;color:#075985;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">📋 Legalitas & SK</div>`;
                if (props.NOSKKWS) html += `<div class="popup-info"><strong>Nomor SK:</strong> ${props.NOSKKWS}</div>`;
                if (props.TGLSKKWS) {
                    let tglDisplay = props.TGLSKKWS;
                    if (props.TGLSKKWS.includes('-')) {
                        try { tglDisplay = new Date(props.TGLSKKWS).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }); } catch(e) {}
                    }
                    html += `<div class="popup-info"><strong>Tanggal SK:</strong> ${tglDisplay}</div>`;
                }
                if (props.INTS) html += `<div class="popup-info"><strong>Instansi:</strong> ${props.INTS}</div>`;
                html += '</div>';
            }
            ...
            if (hasSumberData) {
                html += `<div style="margin-top:10px;padding:10px;background:#f3f4f6;border-radius:6px;">
                    <div style="font-size:11px;font-weight:700;color:#374151;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">📊 Sumber Data</div>`;
                if (props.BPDAS) html += `<div class="popup-info"><strong>BPDAS:</strong> ${props.BPDAS}</div>`;
                const tahun = props.THNBUAT || props.TAHUN;
                if (tahun) html += `<div class="popup-info"><strong>Tahun Data:</strong> ${tahun}</div>`;
                if (props.SMBDT) html += `<div class="popup-info" style="font-size:11px;line-height:1.4;"><strong>Sumber:</strong> ${props.SMBDT}</div>`;
                html += '</div>';
            }

            const remark = props.REMARK || props.NAMA_INSTA;
            const hasValidRemark = remark && remark !== 'Tidak ada catatan' && remark !== 'TIDAK ADA CATATAN' && remark.trim() !== '';
            if (hasValidRemark) {
                html += `<div style="margin-top:10px;padding:8px;background:#fef3c7;border-left:3px solid #f59e0b;border-radius:4px;">
                    <div style="font-size:11px;font-weight:700;color:#92400e;margin-bottom:4px;">📝 CATATAN</div>
                    <div style="font-size:11px;color:#78350f;line-height:1.4;">${remark}</div>
                </div>`;
            }

            html += `<div style="margin-top:10px;padding:6px;background:#f0f0f0;border-radius:4px;font-size:10px;color:#666;">
            ...
            return `<div class="custom-popup">
                ...
                <div style="padding:10px;color:#dc2626;"><strong>Error:</strong> ${error.message}</div>
            </div>`;
        }
    }
    ...
    function onEachFeature(feature, layer, density) {
        layer.bindPopup(createPopupContent(feature), {
            maxWidth: 380, minWidth: 320, maxHeight: 600,
        ...
        layer.on({
            mouseover: function() { this.setStyle({ fillOpacity: 0.7, weight: 3 }); },
            mouseout:  function() { this.setStyle(getStyleByDensity(density)); }
        });
    }
    ...
    async function loadGeoJsonData() {
        ...
        try {
            for (const density in CONFIG.plovisUrls) {
                ...
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status} for ${density}`);
    ...
    function toggleLegend(type) {
        ...
        text.textContent = body.classList.contains('show') ? 'Collapse' : 'Expand';
    }
